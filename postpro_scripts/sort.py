    ...
    for line in align_proc.stdout:
        ...
        if check_window:
            ...
            if discard_multHit: # look all windows with same size
                sted_locIDs = size_sted_locIDs[win_size]
            else:
                sted_locIDs = {} # look only aligned window
                locID = str(win_st) + ':' + str(win_ed)
                sted_locIDs[win_st] = [locID]
                sted_locIDs[win_ed] = [locID]
        ...
        for i in range(len(cigar_str)/2):
            ...
            for j in range(num):
                if read_pt > 0  and ref_pt in sted_locIDs: # not include the first nt of reads
                    for locID in sted_locIDs[ref_pt]:
                        if locID not in locID_readpos:
                            locID_readpos[locID] = []
                        locID_readpos[locID].append(read_pt)
                if s=='M':
                    ref_pt += 1
                    read_pt += 1
                elif s=='I':
        ...
        if read_pt < len(read_seq)  and ref_pt in sted_locIDs: # not include the last nt of reads
            for locID in sted_locIDs[ref_pt]:
                if locID not in locID_readpos:
                    locID_readpos[locID] = []
                locID_readpos[locID].append(read_pt)
        ...
        if check_window:
            candidates = []
            for locID in locID_readpos:
                try:
                    st, ed = sorted(locID_readpos[locID])
                    insert = read_seq[st:ed]
                    if len(insert) != win_size:
                        continue
                    key = insert + '-' + locID.split(':')[0]
                    if key in ref_win:
        ...
        if line.startswith('>'):
            ...
            try: # if library has sensitive windows
                ...
                if check_window and discard_multHit: # gather all window sites if concerning multi-hits
                    if win_size not in size_sted_locIDs:
                        size_sted_locIDs[win_size] = {}
                    locID = str(win_st) + ':' + str(win_ed)
                    if win_st not in size_sted_locIDs[win_size]:
                        size_sted_locIDs[win_size][win_st] = []
                    size_sted_locIDs[win_size][win_st].append(locID)
                    if win_ed not in size_sted_locIDs[win_size]:
                        size_sted_locIDs[win_size][win_ed] = []
                    size_sted_locIDs[win_size][win_ed].append(locID)
